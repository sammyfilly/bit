...
def b58decode(string):
    ...
    try:
        ...
        raise ValueError(f'"{char}" is an invalid base58 encoded character.') from None
...
def b58decode_check(string):
    ...
    if decoded_checksum != hash_checksum:
        raise ValueError(
            f'Decoded checksum {decoded_checksum} derived from "{string}" is not equal to hash checksum {hash_checksum}.'
        )
