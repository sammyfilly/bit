...
def bytes_to_wif(private_key, version='main', compressed=False):
    ...
    prefix = TEST_PRIVATE_KEY if version == 'test' else MAIN_PRIVATE_KEY
    suffix = PRIVATE_KEY_COMPRESSED_PUBKEY if compressed else b''
...
def wif_checksum_check(wif):
    ...
    return decoded[:1] in (MAIN_PRIVATE_KEY, TEST_PRIVATE_KEY)


def public_key_to_address(public_key, version='main'):

    version = TEST_PUBKEY_HASH if version == 'test' else MAIN_PUBKEY_HASH
...
def public_key_to_segwit_address(public_key, version='main'):
    ...
    version = TEST_SCRIPT_HASH if version == 'test' else MAIN_SCRIPT_HASH
    length = len(public_key)

    if length != 33:
        raise ValueError(
            f'{length} is an invalid length for a public key. Segwit only uses compressed public keys'
        )
...
def multisig_to_redeemscript(public_keys, m):
    ...
    for key in public_keys:
        ...
        if length not in (33, 65):
            raise ValueError(
                f'At least one of the provided public keys is of invalid length {length}.'
            )
